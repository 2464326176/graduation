...
'''
两个资源（图像数组和csv文件）
加载到人脸识别的算法中 用于训练人脸识别算法
首先需要创建函数用来朱行读取csv文件  
并将对应路径的图像加载到图像数组中 将ID加载到标签数组中
...
def read_images(path, sz=None):
    ...
    """
    '''
    读取给定文件夹中的图像，如果给出大小，则动态调整图像大小。
    args   path：包含子文件夹的文件夹的路径。
           sz：大小调整大小的元组
    return X：图像，它是numpy数组的Python列表。
           y：Python列表中的相应标签（唯一编号）。
    '''
    c = 0
    X,y = [], []
...
# 如果给出了out_dir，请将其设置为：
# 创建Eigenfaces模型。 我们将使用默认值 thresholdolding：
# model = cv2.face.createLBPHFaceRecognizer()

model = cv2.face.EigenFaceRecognizer_create()


print(model.train(np.asarray(X), np.asarray(y)))
...
# 将第一个（最多）16个特征向量转换为灰度图片。 你也可以在这里使用cv:: normalize，
# NumPy现在要容易得多。注意：特征向量按列存储：
